...
Op1NetworkCatabolism<-function(t, state, parameters) {
  with(as.list(c(state, parameters)),{
    dthr<- ts1*(sustrate/(sustrate+thetats1))+ts2*(sustrate/(sustrate+thetats2))-k*thr-(tha2)*thr/(thr+thetatha)-omr1*(thr/(thr+thetaomr1))*(thetaile/(ile+thetaile))
    dgly<-(tha2)*thr/(thr+thetatha) - k*gly
    dile<-omr1*(thr/(thr+thetaomr1))*(thetaile/(ile+thetaile))-k*ile
    list(c(dthr,dgly,dile))  })
}
...
plotField<-function(coordenadas,t,coordColors,field,x,y,simul){
  ...
  for(i in 1:y){    #ypos
    for(k in 1:x){   #xpos
      ...
      domain[k,i]<-saver}}
  #the following marks the SCN, PD-TD boundary in the RAM, RAM-EZ boundary, EZ-DZ boundary 
