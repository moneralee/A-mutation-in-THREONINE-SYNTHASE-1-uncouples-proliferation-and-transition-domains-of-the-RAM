...
# on threonine The model simulates the expression of TS1 and TSY2 in each cell, and the threonine produced
# in each cell. This threonine is then used as a condition to decide if cells at the RAM (PD) divide or not.

#Code generated by Monica L Garcia-Gomez
#--> Instructions: First run "Threonine-model-Functions.R", and then this code. 
...
dir.create("output")
#Parameters controlling cell growth: Dv (cell length to divide), El (how much cells grow at EZ), Df (cell size at which cells differentiate)
Dv<-3
El<-6
Df<-80
for(h in 1:length(files)){
  ...
  for(t in 1:simulationTime){
    ...
    for(i in 1:id){ 
      simulhelper<-coordenadas[i,17]   #to integrate Thr into the division decision.  
      ...
      if(helper>Df){  #cells differentiate once they reach a certain length
        ...
        if(mod(t,simulationTimegrowthdivision*3)==0){
          if(coordenadas[i,2]==29){
            if( (helper>=5) & (mod(helper,2)!=0) & (simulhelper>umbralRAM)){
              coordenadas[i,6]<-1
        ...
        coordenadas[i,6]<-3
      }else if((coordenadas[i,3]<108) & (coordenadas[i,21]<5) & (simulhelper>umbralRAM)){   #RAM PD- cells divide if they are at PD and have doubled their size, if their division counter has not been surpassed (5max), and if they have free threonine
        if( (helper>=Dv) & (mod(helper,2)!=0)){
          coordenadas[i,6]<-1
        }else{coordenadas[i,6]<-0}
    ...
    for(i in 1:id){
      ...
      state<-c(THR=coordenadas[i,17]) 
      times <-seq(0,10,by=.1)
      o <- ode(y=state,times=times,func=Opnetwork1,parms=parameters) 
      coordenadas[i,17]<-o[101,2]
